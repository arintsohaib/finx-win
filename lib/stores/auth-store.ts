...
interface User {
  ...
  kycStatus?: string; // Add KYC Status
}

interface AuthState {
  isConnected: boolean;
  isLoading: boolean;
  isInitializing: boolean; // New: Track if we're checking for existing session
  user: User | null;
  error: string | null;
  ...
  login: (rawUser: any) => void; // Centralized login with data transformation
}
  ...
  persist(
    (set, get) => ({
      ...
      refreshBalances: async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            } else if (data.user.balances && typeof data.user.balances === 'object') {
              Object.assign(balancesObj, data.user.balances);
            }
            ...
            set(state => ({
              user: state.user ? {
                ...
                balanceDetails: data.user.balanceDetails || state.user.balanceDetails,
                kycStatus: data.user.kycStatus || state.user.kycStatus
              } : null
            }));
          }
        } catch (error) {
      ...
      login: (rawUser: any) => {
        ...
        if (rawUser.balances && Array.isArray(rawUser.balances)) {
          rawUser.balances.forEach((balance: any) => {
            balancesObj[balance.currency] = balance.amount.toString();
            balanceDetailsObj[balance.currency] = {
              total: typeof balance.amount === 'number' ? balance.amount.toString() : balance.amount,
        ...
        // Save session
        sessionManager.saveSession(userData.walletAddress);
      }
    }),
    {
